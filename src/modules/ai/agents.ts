...
export class Agent {
  constructor(readonly type: AgentType, readonly temperature: number, readonly system_prompt: string, readonly model_id: ModelId, readonly enable_tools = false, readonly prefillText = '') { }
  ...
  create(agentType: AgentType) {
    switch (agentType) {
      case AgentType.REACT:
        return new Agent(agentType, 1.0, this.prompts.prompt_react, ModelId.GEMMA_INSTRUCT, /* enable_tools= */ true);
      case AgentType.CODING:
        return new Agent(agentType, 1.0, this.prompts.prompt_coding, ModelId.GEMMA_INSTRUCT, /* enable_tools= */ true);
